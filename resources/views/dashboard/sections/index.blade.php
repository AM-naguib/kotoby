                                ...
                                <tr>
                                    ...
                                    <td class="d-flex gap-3">
                                        <button class="btn btn-primary"
                                            onclick="showModal('edit', {{ $section->id }}, '{{ $section->name }}')">Edit</button>
                                    <button class="btn btn-danger" onclick="deleteItem({{ $section->id }})">Delete</button>
                                    </td>
...
@section('js')
    <script>
        function showModal(type, id = null, name = null) {
            if (type == 'create') {
                $("#modal-title-default").text("Create Section");
                let route = "{{ route('dashboard.sections.store') }}";
                $("#modalBody").html(modalHtml(route, 'POST'));
            }else if (type == 'edit') {
                $("#modal-title-default").text("Edit Author");

                let route = "{{ route('dashboard.sections.update', ':id') }}".replace(':id', id);
        ...
        function deleteItem(id) {
            if (confirm('Are you sure?')) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('dashboard.sections.destroy', ':id') }}".replace(':id', id),
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(data) {
